    ...
    var creationCommands = schemas.map(function (schema) {
      return function (callback) {
        client.indexes.exists(schema.name, function (err) {
          console.log("1st client.indexes.exists", schema.name, 'err ::', err);
          if (! err) return callback();
          client.indexes.create(schema.name, function(err) {
            console.log("1st client.indexes.create", schema.name, 'err ::', err);
            if (err) return callback(err);

            // check if index is really created
            checkReallyCreated(schema.name, callback);
          });
        });
      };
    });

    function checkReallyCreated(schemaName, callback) {
      console.log("checkReallyCreated", schemaName);
      function doCheck() {
        console.log("doCheck");
        client.indexes.exists(schemaName, function (err) {
          console.log("doCheck :: client.indexes.exists", schemaName, 'err ::', err);
          if (! err) return callback();
